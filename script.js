...
function existeMensaje(){//comprueba si hay mensaje para encriptar/desencriptar
    bandera = 0;
    banderamsg=0;
    if(frase == ''){
        bandera++;
    ...
    if(bandera > 0){//no hay mensaje
        ocultar();
        alert("No se encontró ningun mensaje para encriptar/desencriptar");
...
function comprobarMinusculas(){//comprueba si el usuario introdujo minusculas o caracteres especiales y acentos
    bandera1 = 0;
...
function crearIncrementos(){//ayuda en la funcion de "desencriptar".cuantos caracteres debe saltarse si encuentra una llave
    for(let i = 0;i < llaves.length ;i++){
        incrementos[i] = (llaves[i].length - 1);
    }
}

    function encriptar(){//encripta mensajes
        obtenerMensaje();
        existeMensaje();
        comprobarMinusculas();
        fraseEncriptar = [];
        ...
        if(bandera1 == 0){
            ...
            alert("Estas usando caracteres no autorizados, intenta de nuevo");
            ocultar();
            document.getElementById("texto-salida").innerHTML = '';
        }
    }

    function desEncriptar(){//desencripta mensajes
        obtenerMensaje();
        existeMensaje();
        comprobarMinusculas();
        crearIncrementos();
    ...
    function mensajeFinal(texto){//muestra mensaje encriptado/desencriptado
        mensaje = texto[0];
        for(let i = 1;i < texto.length;i++){
            mensaje += texto[i] + '';
        }
        if(banderamsg == 0){//hay mensaje
        //console.log(mensaje)
        document.getElementById("texto-salida").innerHTML = mensaje;
        }
    }

    function ocultar(){//oculta boton "copiar"
        if(ocultarImg.style.display === 'none'){
            ocultarImg.style.display = 'block'; 
        }
    }

    function  mostrar(){//muestra boton "copiar"
        if(ocultarImg.style.display === 'block'){
            ocultarImg.style.display = 'none';
        }
    }

    function copiar(){//copiar el texto encriptado/desencriptado
